...
template <typename CacheItemT, typename KeyT>
class CacheBelady2 {
private:
    // Определение типов для работы с кэшем
    using CacheTree   = std::map<size_t, std::pair<KeyT, CacheItemT>>; // Дерево сортированное по времени до запроса в будущем
    using CacheTreeIt = typename CacheTree::iterator;                  // Итератор для дерева
    using CacheMap    = std::unordered_map<KeyT, CacheTreeIt>;         // Хэш-таблица для быстрого доступа к элементам кэша

    // Типы для работы с данными о будущих запросах
    using RequestsTree   = std::map<size_t, std::pair<KeyT, std::vector<size_t>>>; // Дерево будущих запросов
    using RequestsTreeIt = typename RequestsTree::iterator;                        // Итератор по дереву будущих запросов
    using RequestMap     = std::unordered_map<KeyT, RequestsTreeIt>;               // Связь между ключом и будущими обращениями
...
public:
        ...
        if (isConstructorArgsValid(cacheSize, requestSequence)) {
            ...
            for (size_t i = 0; i < requestSequence.size(); ++i) {
                ...
                if (requestMapIt == requestsMap_.end()) {
                    ...
                    auto emplacePair = requestsTree_.emplace(i, std::make_pair(requestSequence[i], std::vector<size_t>()));
    ...
    bool isCacheFull() const {
        return (cacheTree_.size() == cacheCapacity_);
    }
    ...
    bool lookupUpdate(KeyT key, CacheItemT (*fetchPage)(KeyT)) {
        if (requestsTree_.size() > 0) {
            if (key != requestsTree_.begin()->second.first) {
                throw std::invalid_argument("Key must be equal to the first element in requestsTree");
            }
        }
        ...
        if (cacheMapIt != cacheMap_.end()) {
            ...
            if (isCacheFull()) {
                if (removeUnwantedItem(key) == key) {
                    return false;  // Элемент не нужно добавлять, т.к. он больше не будет запрашиваться
                }
            }
...
public:
    ...
    const CacheTree& getCacheTree() const {
        return cacheTree_;
    }
...
private:
    ...
    bool isConstructorArgsValid(size_t cacheSize, const std::vector<KeyT>& keys) const {
    ...
    void addItemToCache(KeyT newKey, CacheItemT (*fetchPage)(KeyT)) {
        ...
        auto emplacePair = cacheTree_.emplace(distance, std::make_pair(newKey, fetchPage(newKey)));
        if (emplacePair.second) {
    ...
    void updateRequestsTree(KeyT key) {
        ...
        if (requestMapIt == requestsMap_.end()) {
            throw std::invalid_argument("Key must be equal to the first element in requestsMap");
        }
        ...
        if (distanceVector.size() > 0) {
            ...
            auto emplasePair = requestsTree_.emplace(tmpDistance, std::make_pair(key, distanceVector));
            ...
            auto cacheMapIt = cacheMap_.find(key);
            if (cacheMapIt != cacheMap_.end()) {
                auto emplacePair = cacheTree_.emplace(tmpDistance, std::make_pair(key, cacheMapIt->second->second.second));
...
private:
    ...
    RequestsTree requestsTree_;  // Дерево будущих запросов сортированное по времени до запроса
