...
template <typename T, typename K> Cache2Q<T, K>::Cache2Q(size_t sz) {
  if (sz < 3) {
...
template <typename T, typename K> bool Cache2Q<T, K>::lookupUpdateIn(K key) {
...
template <typename T, typename K> bool Cache2Q<T, K>::lookupUpdateHot(K key) {
...
template <typename T, typename K> bool Cache2Q<T, K>::lookupUpdateOut(K key) {
...
void Cache2Q<T, K>::addToIn(K key, F slowGetPage) {
  if (fullIn()) {
    ...
    addToFront(cacheOut_, hashOut_, cacheIn_.back().first,
               cacheIn_.back().second);
